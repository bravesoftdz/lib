...
TstreamingClass=class(TComponent)
  ...
  public
    ...
    constructor LoadFromFile(const filename: string); virtual;  //неужели до меня дошло?
  ...
  if FileExists(filename) then begin
    ...
    try
      ...
      if FileExists(FileName) then
        DeleteFile(FileName);
      RenameFile(BackupName,FileName); //если не переименуется, придется уж юзеру смотреть
      raise;
    end;
  end
  else
