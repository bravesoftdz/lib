...
type
  ComplexTernary2D=class(TPersistent)
    private
      _qx,_qy: Integer; //число тритов по ос€м x и y
...
begin
  ...
  while N1>0 do begin
    T1:=T1 div 3;
    N1:=(T1-1) shr 1;
    big_incr:=incr*3; //дл€ внутреннего цикла
    M1:=(incr-1) div (2*mult); //дл€ внешнего
    ...
    for k:=-N1 to N1 do begin
       ...
       //отдельно обраб. нулевое значение - там не нужно фаз. множителей
        ...
        ydif:=sqrt3*(xp1-xm1);
        xdif:=sqrt3*(ym1-yp1);
        // 4 сложени€ и 2 умножени€ (с плав. точкой)
        Ax:=x0-0.5*xsum;
        Ay:=y0-0.5*ysum;
        // 6 сложений и 4 умножени€
        ...
        //итого, 12 сложений и 4 умножени€
       end;
    //шаг фазового множител€: 2pi/incr;
    //на первой итерации просто 2pi, но там цикл и не запуститс€
    //на второй итер:
    ...
    for i:=1 to M1 do begin
      //пересчитываем фазовый множитель, потом делаем циклы дл€ i и -i
      tmpWr:=Wr;
      ...
      for k:=-N1 to N1 do begin
       ...
       //x0,y0 - без изменений
        ...
        //“еперь, то же самое дл€ элемента -i
...
begin
  w:=btmp.Width;
  h:=btmp.Height;
  wh:=max(w,h); //на первое врем€ (т.е на пару лет :( )
  Set_Length(wh,wh);
  Clear;
  //теперь отцентрируем
