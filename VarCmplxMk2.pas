...
function VarComplexRe(const AValue: Variant): Double;
function VarComplexIm(const AValue: Variant): Double;
...
uses
  VarUtils, SysUtils, Math, SysConst, RTLConsts, TypInfo, Classes, VariantWrapper;
...
type
  ...
  TComplexData = class(TAbstractWrapperData)
  private
    FReal, FImaginary: Double;  
    PoolLink: TComplexData;
  ...
  public
    ...
    class function Create(const AReal: Double): TComplexData; overload;
    class function Create(const AReal, AImaginary: Double): TComplexData; overload;
    class function Create(const AText: string): TComplexData; overload;
    class function Create(const AData: TComplexData): TComplexData; overload;
    ...
    procedure DoAdd(const Right: TAbstractWrapperData);overload; override;
    procedure DoAdd(const AReal, AImaginary: Double);reintroduce; overload;
    procedure DoSubtract(const Right: TAbstractWrapperData); overload; override;
    procedure DoSubtract(const AReal, AImaginary: Double); reintroduce; overload;
    procedure DoMultiply(const Right: TAbstractWrapperData); overload; override;
    procedure DoMultiply(const AReal, AImaginary: Double);reintroduce; overload;
...
var ComplexDataPool: TComplexData;

{ TComplexData }

class function TComplexData.Create(const AReal: Double): TComplexData;
begin
  Result:=TComplexData(TComplexData.GetInstance);
  Result.SetValue(AReal);
end;

class function TComplexData.Create(const AReal, AImaginary: Double): TComplexData;
begin
  Result:=TComplexData(TComplexData.GetInstance);
  Result.SetValue(AReal, AImaginary);
end;

class function TComplexData.Create(const AText: string): TComplexData;
begin
  Result:=TComplexData(TComplexData.GetInstance);
  Result.AsString := AText;
end;
...
procedure TComplexData.Release;
begin
  PoolLink:=ComplexDataPool;
  ComplexDataPool:=self;
end;

class function TComplexData.getInstance: TAbstractWrapperData;
begin
  if Assigned(ComplexDataPool) then begin
    Result:=ComplexDataPool;
    ComplexDataPool:=ComplexDataPool.PoolLink;
  end
  else
    Result:=TComplexData.Create;
end;
...
begin
  if Source.VType = VarType then
    case AVarType of
    ...
    else if TComplexVarData(Source).VComplex.Imaginary=0 then begin
      VarDataInit(LTemp);
      try
      ...
      else inherited //если этот variant custom'ный, то вызвать его Cast
    end
  else
    inherited;
end;
...
begin
  if VarIsComplex(AValue) then
    Result := TComplexVarData(AValue).VComplex.GetAbs
  else
    Result:=AValue;
end;
...
begin
  if VarIsComplex(AValue) then
    Result:=TComplexVarData(AValue).VComplex.Imaginary
  else
    Result:=0;
end;
...
procedure FreeComplexDataPool;
var tmp: TComplexData;
begin
  while Assigned(complexDataPool) do begin
    tmp:=ComplexDataPool;
    ComplexDataPool:=tmp.PoolLink;
    tmp.Free;
  end;
end;
